...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList> averageWorkingOnTasks = new ArrayList<>();
        averageWorkingOnTasks.add(new TaskList("In progress"));
        double averageDays = project.getTaskLists().stream()
                .filter(task -> averageWorkingOnTasks.contains(task))
                .flatMap(list -> list.getTasks().stream())
                .filter(task -> task.getCreated().isBefore(LocalDate.now()))
                .mapToDouble(date -> DAYS.between(date.getCreated(),LocalDate.now()))
                .average()
                .getAsDouble();
        //Then
        Assert.assertEquals(15, averageDays,0);
    }
    ...
    public void testAddTaskListFindUsersTasks(){
        ...
        List<Task> tasks = project.getTaskLists().stream()
                .flatMap(lists -> lists.getTasks().stream())
